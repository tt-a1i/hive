...
| Agent 状态机 | `initializing` / `idle` / `working` / `stopped` 四态（§3.6）。`initializing` 为 PTY 启动后门控态，保护第三方 agent 登录流程——派单入队但不 flush。Gate 释放：`team report ready` / 用户 [Mark Ready] / 超时（默认 30s）。其余由协议事件驱动：send → working、report/cancel → idle（pending_count 归零时）、PTY exit → stopped。**不做超时/卡死检测，不增加心跳** |
