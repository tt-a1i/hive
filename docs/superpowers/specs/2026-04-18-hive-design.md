...
- `status` — 四态之一：`initializing` / `idle` / `working` / `stopped`（详见 §3.6）
...
- **`@<worker_name>` 是 informal 约定**——orch 可在 task 末尾标注 assignee 帮助用户读图，但 spec 不强制语法。UI 渲染 chip 时 fail-soft（精确匹配 workspace 内已存在 worker name 才渲染，否则按 plain text 处理；详见 §6.6.2）
- **缩进表达"子任务分解"**（GFM 原生）——子任务是父任务的拆分单元，**不是依赖关系**；子任务的 assignee 不继承父任务（spec 明示，避免 UI 误染）。任务依赖关系不在 markdown 里建模（详见 §10.2）
...
| 场景 | 触发 | 应对 |
|---|---|---|
| **A. 单 agent 崩溃** | PTY exit code ≠ 0（CLI bug / OOM / 段错误） | UI 标 `exited`，**不自动重启**；按钮 `[Restart]` / `[Delete]`；点 Restart 走 §3.5.3 恢复流程 |
...
| **D. Hive runtime 整体重启** | 用户 cmd+Q、机器重启、`hive` 进程被 kill 后重开 | **所有 agent 卡片显示 stopped**，**不自动启动**（避免一次拉起 N 个 CLI 引发资源风暴）；提供单卡 `[Restart]` + workspace 级 `[Restart All]` 按钮，按钮触发的恢复流程同 §3.5.3 |

B 之外，所有"重启"都走同一个恢复引擎（§3.5.3）。

#### 3.5.2 传输层断开（runtime 不可达）

§3.5.1 是 runtime 视角的 "PTY 进程不在了"；前端视角的对偶问题是 **WebSocket 跟 runtime 断开**：网络瞬断、机器睡眠、用户 cmd+Q 关掉 hive 进程但浏览器还停在页面上。前端**无法区分** runtime 真的崩了、hang 住、网络断几种原因——不能像 §3.5.1 一样按 "PTY exit code" 分场景，只能按"前端跟 runtime 之间还能不能通信"二分。

**前提**：Web UI Server 绑定固定端口、单实例锁（§4.1 / §10.1 #19）——否则用户重启 hive 后 web 重连原 URL 找不到，整个恢复 UX 失效。

**检测**

复用 WS 生命周期，依赖**被动**的 `ws.onclose` / `ws.onerror` 事件，不新增主动 HTTP polling（违背 §3.3.4 / §3.6.3 "不做主动心跳"）。`onclose` 触发后（**首次连接除外**——initial connect 失败不进入 reconnect 语义，由启动引导自行处理）立即在主区顶部显示 inline "Reconnecting…" 指示；指数退避重连（初始 500ms / 倍率 2 / 上限 10s / jitter ±30%，参数标为 tuning 不入 contract）。**无次数上限**，持续指数退避到上限速率（10s/次），告警条始终展示 `[Retry now]` 让用户随时手动加速。

**升级到全屏告警条**的条件是双阈值：累计断开 ≥5s **且** 重试 ≥2 次（时间从 `onclose` 起算，**机器睡眠期间不累计**）——吸收笔电唤醒的瞬断（首次重连通常成功）和 wifi flap（短时多次失败）。纯次数阈值会被毫秒级 flap 误触发；纯时间阈值在长断时延迟过大。

**告警条文案**

> Lost connection to Hive runtime. Restart to recover.
>
> Check the terminal where you started `hive`, or rerun your start command.

文案**不预测 agent 死活**（不说 "agents stopped" / "may still be running"），原因有两层：

- 前端无法判断 runtime 是 crash 还是 hang。runtime crash 时 master fd 关闭确实会发 SIGHUP，**但 hang 住的 runtime 子进程 fd 仍开着、agent 完全活着**——hang 跟 crash 在前端表征上没区别
- 即使收到 SIGHUP，各 CLI 对它的处理各异（少数会 trap），agent 不一定真死

让用户走 Restart 流程后由 §3.5.1 的 D 场景自己说话。按钮单一 `[Retry now]`（手动触发立即重连）；告警条**不可手动 dismiss**——断线持续就持续显示，避免用户误以为已恢复。被 disable 的按钮在 hover 时显示 tooltip `Hive runtime disconnected`，让"看起来没反应"显式化。

实现时建议组件命名为 `ConnectionLostAlert`，与 §12 v0.2 路线规划中 orch 主动推送的通知 banner 区分（不同 UI 角色，避免术语重载）。

**Agent 卡片：connection overlay（UX 表现）**

`connection` 的状态机定位与正交性论证见 §3.6.5。这里只规约 UX 表现：

- `connection = stale` 时卡片整体降饱和 + 降亮度，**仍展示 last-known `status` + `pending_task_count`**（不替换为 "?"——保留信息密度，匹配 Slack/Linear 失联处理惯例）
- 卡片右上角加小图标 + 短文案 "Connection lost"（跟告警条措辞统一）
- 卡片底部加 `last seen Xs ago` 微型时间戳（跟 stopped 卡片显示 "exit code N" 对称——明示"数据已经多久没刷"，避免 "working" 状态因长时间未刷而误导）
- 具体视觉规则后续在 §6.3 卡片状态视觉中展开（本次不改）

**重连后恢复：snapshot-then-resume**

WS 重连成功后，客户端发起一次 `state.snapshot` 请求拉取 authoritative state：runtime `instance_id`、每个 agent 的 `status` + `pending_task_count`、当前 workspace 的 messages tail、`.hive/tasks.md` 内容。snapshot 一次性带回当前状态，覆盖前端缓存，恢复正常协议事件订阅。

`state.snapshot` 的具体 RPC 契约（method 名、payload schema、错误码）由实现期落地到 §4 / §5；本节只定义 UX 行为契约。

**Instance ID 判定**（决定重连成功后的视觉路径）：客户端比对 snapshot 返回的 `instance_id` 跟断开前缓存的：

- **一致** → runtime 是同一进程（中间只是网络/WS 链路波动），沿用原 `status`，撤销 stale overlay，告警条消失
- **不一致** → runtime 已重启（用户 cmd+Q 后又起、或 crash 后被某种方式拉起），snapshot 拉回的所有 agent 都是 stopped——视觉等同于 §3.5.1 D 场景：所有卡片显示 stopped + 提供 `[Restart All]`，告警条消失

**Terminal scrollback** 的回放复用参考项目 kanban / cmux 的"服务端 terminal mirror"思路（具体实现细节交给后续模块文档）：每个 PTY 在服务端维护 N 行 scrollback，重连时全量重放——浏览器看到的 terminal 跟没断过一样。

**边界澄清**：§3.5.3 Layer B 摘要的"不抓不存 PTY transcript / agent 输出字节流"指的是**恢复语义层**约束（构造接力上下文时不消费历史字节），跟这里的**显示层** scrollback 缓冲是两件事——前者是"重启后给新 agent 看什么"，后者是"重连后给浏览器看什么"，两者不冲突。

**不做** server-buffered protocol event replay——断开期间的协议事件已落 `messages` 表，snapshot 一次性带回当前状态足够，不必逐事件回放（与 §3.5.3 Layer A "完整恢复对话历史"思想一致：状态优先于事件流）。

**断线期间的写操作禁用（principled disable）**

原则：所有"写 runtime 状态"的入口禁用。具体（不穷举）：

- Workspace 级：`[Add Workspace]` / `[Delete Workspace]` / 角色模板编辑（§3.2）/ 启动配置编辑（§7.1）
- Agent 级：`[Add Worker]` / `[Restart]` / `[Restart All]` / `[Stop]` / `[Delete]`
- 派单与输入：UI 派单入口（如有）、**PTY stdin 写入**（用户在 terminal 敲键被前端拦截，toast 提示）

保持可用（**只读 + 客户端**操作）：

- 滚动 terminal scrollback（前端缓存）
- 查看 task graph（cached `.hive/tasks.md`，断线前最后一次拉到的内容）
- workspace / sidebar 导航、主题、设置
- 复制按钮（terminal output、消息正文等纯文本）

**Multi-tab 假设**

MVP **不做 tab 间协调**：每个 tab 各自维护 WS 连接、各自感知 disconnect / reconnect、各自显示告警条与卡片状态。tab A 重连不影响 tab B 视图。**单 tab 内 WS 是 runtime 全局连接**（不分 workspace）——断开时该 tab 下所有 workspace 卡片同时进入 `connection = stale`。多 tab 协调（如"tab A 重连应让 tab B 也刷新"）出 MVP 范围（见 §10.2）。

§3.5.1 与本节互补：§3.5.1 处理"runtime 健在、PTY 不在了"，§3.5.2 处理"runtime 失联、PTY 状况未知"。两者**都不直接触发** §3.5.3 的 Layer A/B 恢复——传输层重连若 `instance_id` 一致，runtime 内部 agent 进程没动；若 `instance_id` 不一致则等价于 §3.5.1 D，按 D 流程走。
...
- **不消费**也不试图恢复：
  - PTY transcript / agent 输出字节流（不抓不存，详见 §3.5.3 论证）
...
UI 想展示的 worker 状态必须跟协议能力对齐——只看 initializing / working / idle / stopped 四态，不假装能感知 agent 内部的"卡住"或"出错"。
...
```
              ...
              └──────┬───────┘
                     ...
                     ▼
              ┌──────────────┐
              │ initializing │  ← PTY 刚启动，agent 可能处于登录/初始化交互中
              │              │     派单队列暂挂：消息入队但不 flush 到 stdin
              └──────┬───────┘
                     │ Gate 释放（三选一，先到先得）：
                     │   a) agent 调 `team report ready`
                     │   b) 用户点 UI [Mark Ready]
                     │   c) per-role 启动超时到期（默认 30s，可配置）
                     ▼
              ┌──────────────┐
...
> **设计动机**：第三方 agent（Aider、Codex 等）spawn 后有登录/初始化交互流程。若在此期间注入 `team send` 内容，会被 login prompt 吞掉或打断认证流。`initializing` gate 确保派单只在 agent 就绪后才 flush，且不违反核心原则——不解析 stdout、不加心跳、不做静默检测。

#### 3.6.2 状态来源（哪个事件触发哪个转移）

| 状态/转移 | 触发事件 | 数据来源 |
|---|---|---|
| `stopped` → `initializing` | PTY 启动 | runtime 监听 `node-pty` spawn 回调 |
| `initializing` → `idle` | Gate 释放：`team report ready` / 用户 [Mark Ready] / 启动超时到期 | Team Command Server 收到 ready report；UI 按钮事件；runtime 计时器 |
| `initializing` → `stopped` | PTY exit（初始化期间崩溃） | `node-pty` onExit 回调 |
...
> **队列行为**：`initializing` 期间收到的 `team send` 正常入队并递增 `pending_task_count`，但**不 flush 到 stdin**。Gate 释放瞬间，队列按 FIFO 顺序逐条注入。因此 orch 在 worker 初始化完成前就可以派单，不会丢失。
...
#### 3.6.5 第三个正交维度：`connection`

WS 跟 runtime 断开时，前端无法区分 runtime 真崩、hang、网络断这几种原因（详见 §3.5.2），状态机层也不能产生新转移（没有协议事件）。展示层补一个**第三个正交维度** `connection`（`live` / `stale`），表达"上次已知 `status` 是否仍可信"。

| 维度 | 来源 | 值 |
|---|---|---|
| `status`（§3.6.1） | 协议事件（send / report / PTY exit） | `working` / `idle` / `stopped` |
| `pending_task_count`（§3.6.4） | 内存计数 | 整数（≥0） |
| `connection`（本节） | 前端 WS 生命周期（`onclose` / 重连成功） | `live` / `stale` |

- `connection` **不参与** §3.6.1 状态机转移判定——纯展示层属性
- 三个维度各自独立来源、组合渲染，避免把传输层信号污染进 §3.6.1 四态状态机（跟 §3.6.4 的 `status × pending_task_count` 正交模式同款）
- **适用范围**：`connection` overlay 同时作用于 **worker 卡片层**（§6.3）和 **任务图抽屉层**（§6.6.4）；stale 时两处一致降饱和
- `connection = stale` 时的卡片渲染规则、双阈值检测、`state.snapshot` 重连协议、`instance_id` 判定都在 §3.5.2
...
- **Web UI Server**：本地 HTTP + WebSocket server，绑定 `127.0.0.1`，**固定默认端口**（具体值由实现选定；用户可在配置里覆盖；启动时 EADDRINUSE 直接退出并报错）。Web UI 静态资源 + tRPC + WS 终端流都走这个 server。固定端口是 §3.5.2 "Restart Hive to recover" 文案的隐含前提——重启后浏览器原 URL 必须仍可达。
- **Runtime Instance ID**：runtime 启动时生成一个 `instance_id`（UUID 或 unix-ms timestamp），通过 `state.snapshot` 等接口暴露给 web。前端用它判断 WS 重连命中的是不是同一个 runtime（详见 §3.5.2）。
...
│ ● working           │ ← 状态指示（四态：initializing / idle / working / stopped，详见 §3.6）
│ 「实现登录接口」    │ ← 当前任务摘要（最近一次 team send 的 text 截断；idle 时显示 "—" 或最近一次 report 摘要）
│ 队列: 0             │ ← 待执行任务数（pending_task_count，详见 §3.6.4）
└────────────────────┘
```

**头像**：显示 worker 所用 CLI agent 的官方 logo（Claude Code / Codex / OpenCode / Gemini），让用户在 6 张卡片里一眼区分团队的 CLI 构成。映射来源是 `team list` payload 新加的 `command_preset_id` 字段（仅暴露 4 个内置 preset 之一），优先取 `agent_launch_configs.command_preset_id`，缺失时按 `config.command` 反查内置注册表（与 `agent-run-bootstrap.ts::resolveLaunchPreset` 同源）。当 worker 跑的是自定义 `command`、launch config 行不存在、或 UI 尚未识别新 preset 时，**fall back 到现有的 role-letter avatar**（Coder → "Co" 等），保证升级前的存量 worker 永远不会显示空白。logo 资源以 `web/public/cli-icons/` 静态文件形式部署，浅色背景方块兜底 OpenCode 黑白棋盘在深色主题下的对比度（参考 golutra README 头像样式）。
...
**任务图抽屉联动**（详见 §6.6.6）：
- 点击 任务图抽屉里的 assignee chip 时，对应 worker 卡片 **scroll-into-view + 1s 高亮**
- hover chip 时仅高亮卡片（不 scroll），避免无意触发页面跳动
- **不做反向**（worker 卡片 → 任务图抽屉 task 高亮）：卡片"当前任务摘要"字段已经覆盖反向需求
...
下面是渲染视图的视觉与交互规范（**L1 = M2 首批落地的视觉+交互范围**）——所有规则只在前端 UI 生效；`.hive/tasks.md` 保持纯 GFM task list，markdown 是 source of truth。

#### 6.6.1 层次与缩进

- **Indent guide line**：每级嵌套加一条 subtle 灰色竖线，让父子关系一眼可见
- **视觉深度限制 3 级**：4 级及以上的子任务统一前缀 `▸`，不继续视觉嵌套（鼓励 orch 在 markdown 层就扁平化深嵌套）
- **长行换行**：单行任务文本超过抽屉宽度时换行，**第二行起左侧与文本起点对齐**（不与 checkbox 对齐），避免视觉断层

#### 6.6.2 Assignee chip 与 `@` 语法

- `@<worker_name>` 渲染为 **图标 + 名字 chip**，颜色由 hash-based palette 决定（见 `web/src/tasks/task-meta.ts` 的 `ownerToneFromName`——同一 worker name 跨视图始终相同 tone，给眼睛"稳定锚点"）；palette 8 色循环，6+ worker 时可能 collision（**collision 时 deterministic**：相同名字相同颜色），跟 worker 卡片配色统一推到 v0.2
- **解析 fail-soft**：仅在能精确匹配到当前 workspace 内已存在的 worker name 时渲染 chip；`email@example.com` / `@Unknown` / `@alice's task` 等 false-positive 一律按 plain text 渲染，不打断阅读
- §3.4 中 `@<worker_name>` 是 informal 约定——orch 自主判断要不要在 task 末尾标 assignee；**子任务无 `@` 时不继承父**，按未指派处理（避免"父归 Alice，子任务被误显示成 Alice 在做"）

#### 6.6.3 完成态与进度

- `[x]` 任务：文本 + chip 整体灰化 + strikethrough，视觉权重明显降低
- 父任务 `[x]` **不联动**子任务（沿用 GFM 原生语义；orch 自己负责一致性）
- 父任务右侧小字 `N/M` 显示**直接** checkbox 子任务的完成数：仅一层、非递归；非 checkbox 的 bullet（`-` 但没 `[ ]`）不算；`0/0` 不显示徽章；纯黑白文字不上色（避免引入新色系）

#### 6.6.4 Connection stale 适配

- §3.5.2 transport disconnect 期间，`connection = stale` 时**整个抽屉降饱和 + 降亮度**（跟 worker 卡片对称——§3.6.5 connection overlay 适用范围 = 卡片层 + 抽屉层，本次扩展）
- 抽屉内 task 仍展示 cached `.hive/tasks.md` 内容（断线前最后一次拉到的；见 §3.5.2 "保持可用"）
- assignee chip 的 avatar 随 chip 一同降饱和
- **写入路径 disable**（沿用 §3.5.2 principled disable）：stale 期间 checkbox `Space` 切换、行内编辑、原始 markdown 编辑器"保存"都禁用；`[复制]` 按钮和折叠/展开按钮（纯 UI 状态）保持可用

#### 6.6.5 折叠 / 展开（in-memory only）

- 父任务行加 `▾` / `▸` 按钮折叠 / 展开嵌套子任务
- **折叠状态仅前端内存**：刷新 / 关 tab / 切 workspace / WS 重连后的全量 snapshot 都会重置回全展开，**不写文件、不入库、跨 reload 不保留**
- 外部 edit（orch 改文件）触发重渲染时不尝试 reconcile 旧的 collapsed set——直接全展开

#### 6.6.6 行交互

- 鼠标 hover 一行：**右侧**仅显示一个 `[复制]` 按钮（复制该行 markdown 原文，方便 quote 给 orch）——不堆其他操作
- 点击 assignee chip → 对应 worker 卡片 **scroll-into-view + 1s 高亮**；**hover chip 不 scroll**（只 hover 高亮卡片本身），避免无意触发页面跳动
- 点击 task 文本主体 → 行内编辑（沿用本节顶部规则）
- **不做反向**：worker 卡片 → 抽屉 task 高亮（worker 卡片已有"当前任务摘要"字段 §6.3，反向价值低）

#### 6.6.7 Markdown 内联与键盘可达

- task body 内的 inline code `` `code` `` 和 **bold** 正常渲染（`POST /login` / `**重要**` 等常见）；**link `[text](url)` 不自动渲染为可点击**——避免误点带飞用户，保留 markdown 原文样式
- `Tab` / `Shift+Tab` 焦点移动、`Esc` 关闭抽屉、checkbox `Space` 切换
- `j/k` / `/` 搜索等 power-user 快捷键 **不在 L1 范围**（v0.2 后视用户反馈再考虑）

#### 6.6.8 空状态

- `.hive/tasks.md` 不存在或为空时，抽屉显示空状态卡，两条路径并列（不暗示必须由 orch 主导）：

  > 这个 workspace 还没有任务图。
  >
  > 让 orchestrator 帮你规划：在对话区告诉它你想做什么。
  >
  > 或者手动创建 `.hive/tasks.md` 开始一个 GFM task list。

#### 6.6.9 显式 Out of Scope（L1 不做、v0.2 再议）

- **Per-task 状态色带**（按 assignee 当前 worker status 给 task 染色）—— 四色映射跟 §3.6 四态语义不直接对齐；未指派 / 多 assignee / `[x]` 但 assignee 正在干别的任务等情况未定义；先不上避免引入 ambiguous 视觉契约
- **Chip 颜色跟 worker 卡片配色统一** —— 当前 chip 颜色用 8 色 hash palette 独立运作（§6.6.2），跟卡片配色不同源；统一两边色源（chip 颜色 = 卡片颜色）推到 v0.2
- 其他任务图抽屉相关 OoS 项（拖拽改顺序 / DAG 渲染 / 折叠持久化 / worker → task 反向联动 / 自定义 task ID）见 §10.2
...
command_presets {
  ...
  // §3.5.3 Layer A: CLI 原生 session resume 配置（null 表示不支持，走 Layer B 兜底）
...
agents {
  ...
  last_session_id: string | null  // §3.5.3 Layer A: 启动后捕获的 CLI 原生 session ID
  created_at
}
...
17. ✅ **Agent 状态机**：`initializing` / `idle` / `working` / `stopped` 四态（详见 §3.6）；`initializing` gate 保护第三方 agent 登录流程；不做超时/卡死检测，不增加心跳协议
18. ✅ **Web↔runtime 传输层断开 UX**：WS 重连 + 顶部告警条 + 卡片 `connection` overlay + 写操作 disable（详见 §3.5.2）；不做 `/api/health` 轮询、不做心跳协议（沿用 §3.6.3 既有约束）
19. ✅ **单实例锁 + 固定 Web UI 端口**：Web UI Server 绑定 `127.0.0.1` 上的**固定默认端口**（启动时 EADDRINUSE 直接退出并报错，不悄悄 fallback 到随机端口）；保证 #18 的 "Restart Hive to recover" 文案语义可信——浏览器原 URL 在 runtime 重启后仍指向同一端口
20. ✅ **Runtime Instance ID**：runtime 每次启动生成新 `instance_id`，通过 `state.snapshot` 暴露；前端比对判断 WS 重连命中的是不是同一 runtime，决定 stale overlay 撤销 vs 切换到 §3.5.1 D 视觉（详见 §3.5.2）
...
- ❌ **任务图依赖建模**（缩进 = 子任务分解，**不是**依赖关系；不引入 `[after:#id]` / frontmatter 等依赖语法；详见 §3.4 / §6.6.9）
- ❌ **任务图抽屉拖拽改顺序**（用户要改顺序请走 markdown 编辑器模式；避免落入 "DAG 编辑器" 边界）
- ❌ **自定义 task ID / 标签语法**（保持 GFM 纯净——任何 `id:` / `tags:` / `priority:` 等扩展属性都不在 MVP 范围）
- ❌ **折叠状态跨会话持久化**（§6.6.5 明示：仅 in-memory，跨 reload 重置；不写文件不入库）
- ❌ **Worker 卡片 → 任务图抽屉 task 反向高亮**（§6.6.6：卡片"当前任务摘要"字段已覆盖反向需求）
- ❌ **Per-task 状态色带**（§6.6.9：四色映射跟 §3.6 四态不对齐；推 v0.2）；chip 颜色已用 hash palette 实现，跟卡片配色统一推 v0.2
- ❌ **同屏多 workspace**（拆分窗口/网格——一次只看一个）
- ❌ 用户认证（本地单用户）
- ❌ 远程访问（绑定 127.0.0.1）
- ❌ 任务图模板市场
- ❌ Agent 性能/成本统计
- ❌ **跨 tab 协调**（多 tab 间状态广播 / disconnect 同步 / WS 连接复用等）：每个 tab 自治维护 WS 与 disconnect 状态（详见 §3.5.2）
...
| Web 还活着但 runtime 已死/失联，用户以为 hive 还在工作 | §3.5.2：WS 断线检测 + 顶部告警条明示 "Lost connection to Hive runtime" + 卡片 `connection` overlay 灰化 + 写操作 disable，让"看起来坏了"在 UX 层显式化；告警条文案不预测 agent 死活（无法区分 runtime crash vs hang vs 网络断） |
| Web UI 默认端口被其他进程占用导致 runtime 启动失败 | §4.1 / §10.1 #19：启动时 EADDRINUSE 直接退出并给清晰错误（建议另起端口或停掉占用进程），**不**悄悄 fallback 到随机端口（会让 §3.5.2 "Restart Hive to recover" 文案失效） |
| 用户同时跑多个 hive 实例造成端口冲突 / web 连错 runtime | §10.1 #19 单实例锁兜底（EADDRINUSE 拒绝第二实例）；多实例对运行中 agent 的语义混乱不在 MVP 解决，留给后续 |
| 任务图抽屉 assignee chip parser 把 `email@example.com` / `@Unknown` 等误识别为 worker | §6.6.2 fail-soft：仅在能精确匹配 workspace 内已存在 worker name 时才渲染 chip，否则按 plain text 显示——不打断阅读，渲染失败不影响 task list 可用性 |
| 任务图抽屉视觉密度过载（indent guide + checkbox + chip + `N/M` + `[复制]` + 完成态降权 + stale 滤镜叠加导致色彩黑洞） | §6.6 各子节明示视觉分层规则：状态指示、归档（[x]）、连接 stale 三层 CSS 滤镜不互相打架；chip 当前不带颜色，避免色彩冲突 |
