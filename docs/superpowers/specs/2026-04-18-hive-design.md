...
CLI agent 的内部状态（对话历史、内部推理、工具调用栈）**只活在它自己的进程内存里**。Hive 不试图持久化或重放 PTY transcript，因为：

- PTY 字节流是 ANSI escape 序列，不是干净文本，反向解析成本高
- 中等会话动辄数 MB，全量重放触发 token 成本爆炸 / 超 context limit
- agent 收到"假装是自己之前说过的话"的输入，可能**重复执行 Edit/Bash 工具**导致破坏
- CC/Codex/Gemini 自身崩溃也不恢复，跟产业惯例一致

Hive 的恢复模型 = **重启 + 摘要换班（不是真"恢复"，而是"接力"）**：

**触发**：
- agent PTY 进程退出（exit code != 0）/ 用户手动点 [Restart] 卡片按钮
- UI 把卡片标 `exited` + 显示 [Restart] / [Delete] 按钮，**不自动重启**

**重启流程**：
1. Hive 启动新 PTY，复用同一 `agent_id` 和启动命令
2. 等 PTY 就绪（首次 prompt 出现）
3. 系统从 SQLite messages + 当前 tasks.md + 现有 worker 列表拼装"前情摘要"
4. 通过 stdin 注入摘要给新 agent（格式同 §3.3.2 派单消息模板，但是 system 角色）

**摘要内容**（由系统生成，不依赖 agent）：
...
[Hive 系统消息：你是 <workspace> 的 <角色>。
 ...
 ## 你已派出的任务（最近 N 条 send，未收到对应 report 的优先列出）
 - 派给 Alice: "..." → 等待中（已 8 分钟）
 ...
 请基于此继续工作。如果不确定，问 user。]
```

**为什么这套够用**：
- `tasks.md` 是任务图的 SSOT（source of truth），它在文件系统不丢
- messages 表记录了所有 send/report/user_input，足够构造摘要
- worker 列表 + 当前状态在 SQLite agents 表，重启不丢
- agent 重启后能用 `team list` 自查、读 `tasks.md` 自查、问 user 确认——它会自己补齐细节

**Worker crash 同样处理**——区别只是 worker 的"前情摘要"主要是：最近收到的派单 + 自己 report 过的内容 + 当前 tasks.md 中跟自己相关的项。
...
5. **摘要长度 / token 预算**：crash 重启时摘要拼装的具体长度上限、消息条数上限，需要根据实际 agent 行为调优
...
| Crash 重启后 agent 上下文丢失 | 不假装做"恢复"，明确走"摘要换班"模型（§3.5）；用户预期管理 + 手动 [Restart] 按钮 |
| 重启时 agent 重复执行已完成的工具调用 | 摘要里明确区分"已 report 完成的任务" vs "等待中的任务"；agent 凭文件状态自查（git status / read tasks.md）确认实际进度 |
