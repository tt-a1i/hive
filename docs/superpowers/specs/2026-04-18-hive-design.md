...
2. **错误恢复**：worker PTY crash 怎么处理？UI 提示 + 卡片标 error，用户决定重启或删除
3. **Orch crash**：可重启，对话历史从 SQLite messages 表回放
4. **同名 agent**：UI 层校验，不允许同 workspace 内重名（不同 workspace 间可重名）
5. **`team send` 后 worker 还在忙**：进入 per-agent 队列（golutra 的 32 项上限）
6. **如何让 CLI agent "知道"自己能用 `team` 命令**：第一次启动时往 PTY stdin 注入一段 system 消息，说明可用命令列表
