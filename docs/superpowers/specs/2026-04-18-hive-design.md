...
- 三个维度各自独立来源、组合渲染，避免把传输层信号污染进 §3.6.1 三态状态机（跟 §3.6.4 的 `status × pending_task_count` 正交模式同款）
...
- **Per-task 状态色带**（按 assignee 当前 worker status 给 task 染色）—— 四色映射跟 §3.6 三态语义不直接对齐；未指派 / 多 assignee / `[x]` 但 assignee 正在干别的任务等情况未定义；先不上避免引入 ambiguous 视觉契约
...
- ❌ **Per-task 状态色带**（§6.6.9：四色映射跟 §3.6 三态不对齐；推 v0.2）；chip 颜色已用 hash palette 实现，跟卡片配色统一推 v0.2
