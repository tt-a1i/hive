...
- **PTY Pool**：`node-pty` 跑的所有终端会话，按 workspace 分组。每个 agent 有专属环境变量（`HIVE_PORT`、`HIVE_PROJECT_ID`、`HIVE_AGENT_ID`），让 `team` 命令能定位"我在哪个 workspace、我是谁、汇报给谁"。Agent 的 cwd = 该 workspace 的项目根。
...
5. **同名 agent**：UI 层校验，不允许同 workspace 内重名（不同 workspace 间可重名）
