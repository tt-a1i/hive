...
- 描述：维护项目根 `tasks.md`、根据 user 需求拆解任务、调度 worker；可调 `team send / team list / team report`
...
跟角色模板正交的另一组预置：**启动命令** —— CC / Codex / OpenCode / Gemini，都默认带 YOLO 参数（CC `--dangerously-skip-permissions`、Codex 等同 flag）。用户可填自定义命令。详见 §7.1 的 `command_presets` 表。
...
- **只消费** `messages` 表中的 `type ∈ {user_input, send, report}` 三类记录 + `tasks.md` 当前文件 + `team list` 等价的 worker 列表（`agents` 表查 + 内存里的 `pending_task_count`）
- **不消费**也不试图恢复：
...
- **不需要心跳/进度上报**：协议保持 send/report/list 三命令，不增加 `team status`
...
> **[M2 scope 调整]** `tasks.md` watcher 在 M2 未实现，延后到 **M3**。M2 只提供 tasks API + UI 读写，不做文件实时监听/广播。
...
6. Alice 在 PTY 里执行: team report "已实现 POST /login" --success --artifact src/auth.ts
   ...
   ↓ HTTP POST 127.0.0.1:HIVE_PORT/report
              { project_id, from_agent_id: alice_id, token, result, status, artifacts }
...
9. △ 任务图 = `tasks.md`（每个 workspace 独立），UI 渲染 + 编辑器
